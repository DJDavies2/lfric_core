...
contains 
  ...
  subroutine num_dof_init( mesh, k, w_unique_dofs, w_dof_entity )
    ...
    integer, intent( out ) :: w_unique_dofs(7,4) ! there are 5 fn-spaces
    integer, intent( out ) :: w_dof_entity(7,0:5)
    ...
    integer :: nw0,nw0_cell,nw0_face,nw0_edge,nw0_vert,            &
               ...
               nw2v,nw2v_cell,nw2v_face,                           &
               nw2h,nw2h_cell,nw2h_face
    ...
    integer :: nw0_exterior,nw0_interior,nw1_exterior,nw1_interior,&
               ...
               nw2v_exterior,nw2v_interior,nw2h_exterior,nw2h_interior
    ! global numbers of unique dofs
    integer :: nw0_g, nw1_g, nw2_g, nw3_g, nwtheta_g, nw2v_g, nw2h_g
    ...
    ! Special case for w2v-space, for vertical: F_exterior=1 and F_in=0
    !                          and for horz: F_exterior=0 and F_in=2
    ! Just include the vert-direction for the moment
    F_exterior=1
    F_interior=0
    nw2v  = 1*(k+2)*(k+1)*(k+1)
    nw2v_cell = 1*k*(k+1)*(k+1)
    nw2v_face =     (k+1)*(k+1)
    nw2v_exterior = 0*V_exterior + 0*E_exterior + nw2v_face*F_exterior
    nw2v_interior = nw2v_cell     + 0*E_interior + nw2v_face*F_interior

    ! Special case for w2h-space, for vertical: F_exterior=0 and F_in=4
    F_exterior=0
    F_interior=4
    nw2h  = 2*(k+2)*(k+1)*(k+1)
    nw2h_cell = 2*k*(k+1)*(k+1)
    nw2h_face =     (k+1)*(k+1)
    nw2h_exterior = 0*V_exterior + 0*E_exterior + nw2h_face*F_exterior
    nw2h_interior = nw2h_cell    + 0*E_interior + nw2h_face*F_interior
    ...
    w_unique_dofs(6,1) = nw2v_g
    w_unique_dofs(7,1) = nw2h_g
    ...
    w_unique_dofs(6,2) = nw2v
    w_unique_dofs(7,2) = nw2h
    ...
    w_unique_dofs(6,3) = nw2v_exterior
    w_unique_dofs(7,3) = nw2h_exterior
    ...
    w_unique_dofs(6,4) = nw2v_interior
    w_unique_dofs(7,4) = nw2h_interior

    ! Number of dofs per mesh entity for each space
    ndof_entity_w0(:) = (/ nw0_vert, nw0_edge, nw0_face, nw0_cell,        &
                        nw0_exterior,nw0_interior /)
    ndof_entity_w1(:) = (/ 0       , nw1_edge, nw1_face, nw1_cell,        &
                        nw1_exterior,nw1_interior /)
    ndof_entity_w2(:) = (/ 0       , 0       , nw2_face, nw2_cell,        &
                        nw2_exterior,nw2_interior /)
    ndof_entity_w3(:) = (/ 0       , 0       , 0       , nw3_cell,        &
                        nw3_exterior,nw3_interior /)
    ndof_entity_wtheta(:) =(/ 0   , 0       , nwtheta_face, nwtheta_cell, &
                           nwtheta_exterior, nwtheta_interior /)
    ndof_entity_w2v(:) = (/ 0      , 0       , nw2v_face, nw2v_cell,      &
                         nw2v_exterior, nw2v_interior /)
    ndof_entity_w2h(:) = (/ 0      , 0       , nw2h_face, nw2h_cell,      &
                         nw2h_exterior, nw2h_interior /)
    ...
    w_dof_entity(5,:) = ndof_entity_wtheta(:)
    w_dof_entity(6,:) = ndof_entity_w2v(:)
    w_dof_entity(7,:) = ndof_entity_w2h(:)
    ...
    write( log_scratch_space, '(a,i6,a,i6,a,i6,a,i6,a,i6,a,i6,a,i6)' ) &
        ...
        '   ', nw3_cell, '   ', nwtheta_cell, '       ', nw2v_cell, '    ',    & 
        nw2h_cell
    call log_event( log_scratch_space, LOG_LEVEL_INFO )
    write( log_scratch_space, '(a,i6,a,i6,a,i6,a,i6,a,i6,a,i6,a,i6)' ) &
        'dof on face    ', nw0_face, '   ', nw1_face, '   ', nw2_face, '   ',  &
        0, '   ', nwtheta_face, '       ', nw2v_face, '    ', nw2h_face
    call log_event( log_scratch_space, LOG_LEVEL_INFO )
    write( log_scratch_space, '(a,i6,a,i6,a,i6,a,i6,a,i6,a,i6,a,i6)' ) &
    ...
    call log_event( log_scratch_space, LOG_LEVEL_INFO )
    write( log_scratch_space, '(a,i6,a,i6,a,i6,a,i6,a,i6,a,i6,a,i6)' ) &
        'dof on exterior', nw0_exterior, '   ', nw1_exterior, '   ',           &
        nw2_exterior, '   ', nw3_exterior, '   ', nwtheta_exterior, '       ', &
        nw2v_exterior, '    ', nw2h_exterior
    call log_event( log_scratch_space, LOG_LEVEL_INFO )
    write( log_scratch_space, '(a,i6,a,i6,a,i6,a,i6,a,i6,a,i6,a,i6)' ) &
        'dof on interior', nw0_interior, '   ', nw1_interior, '   ',           &
        nw2_interior, '   ', nw3_interior, '   ', nwtheta_interior, '       ', &
        nw2v_interior, '    ', nw2h_interior
    call log_event( log_scratch_space, LOG_LEVEL_INFO )
