...
! #1001 will implement algortihm layer calls to the kernel
  ...
  !----------------------------------------------------------------------------
  !> Handles the passing of coarse and fine fields to the restrict kernel.
  !> The mesh map shape arguments are not yet supported by PSyClone,
  !> where it is described by Issue #1173.
  !> https://github.com/stfc/PSyclone/issues/1173
  subroutine invoke_restrict_kernel( coarse, fine )
    ...
    ! Arguments
    type(field_type), intent(in) :: coarse, fine
    ! Internal variables
    integer(kind=i_def) :: cell
    integer(kind=i_def) :: nlayers
    integer(kind=i_def) :: ndf_coarse, undf_coarse, ndf_fine, undf_fine
    integer(kind=i_def) :: ncell_fine, ncell_f_per_c_x, ncell_f_per_c_y
    integer(kind=i_def), pointer :: map_coarse(:,:) => null()
    integer(kind=i_def), pointer :: map_fine(:,:) => null()
    integer(kind=i_def), pointer :: cell_map_coarse(:,:,:) => null()
    type(mesh_map_type), pointer :: mmap_fine_coarse => null()
    type(mesh_type),     pointer :: mesh_fine => null()
    type(mesh_type),     pointer :: mesh_coarse => null()
    type(field_proxy_type)       :: coarse_proxy
    type(field_proxy_type)       :: fine_proxy

    !
    ! initialise field and/or operator proxies
    !
    coarse_proxy = coarse%get_proxy()
    fine_proxy = fine%get_proxy()
    !
    ! Initialise number of layers
    !
    nlayers = coarse_proxy%vspace%get_nlayers()
    !
    ! Look-up mesh objects and loop limits for inter-grid kernels
    !
    mesh_fine => fine_proxy%vspace%get_mesh()
    mesh_coarse => coarse_proxy%vspace%get_mesh()
    mmap_fine_coarse => mesh_coarse%get_mesh_map(mesh_fine)
    cell_map_coarse => mmap_fine_coarse%get_whole_cell_map()
    ncell_fine = mesh_fine%get_last_halo_cell(depth=2)
    ncell_f_per_c_x = mmap_fine_coarse%get_ntarget_cells_per_source_x()
    ncell_f_per_c_y = mmap_fine_coarse%get_ntarget_cells_per_source_y()
    !
    ! Look-up dofmaps for each function space
    !
    map_coarse => coarse_proxy%vspace%get_whole_dofmap()
    map_fine => fine_proxy%vspace%get_whole_dofmap()
    !
    ! Initialise number of DoFs for coarse
    !
    ndf_coarse = coarse_proxy%vspace%get_ndf()
    undf_coarse = coarse_proxy%vspace%get_undf()
    !
    ! Initialise number of DoFs for fine
    !
    ndf_fine = fine_proxy%vspace%get_ndf()
    undf_fine = fine_proxy%vspace%get_undf()
    !
    ! Call kernels and communication routines
    !
    ...
    do cell=1,mesh_coarse%get_last_edge_cell()
      call restrict_kernel_code(nlayers,                   &
                                cell_map_coarse(:,:,cell), &
                                ncell_f_per_c_x,           &
                                ncell_f_per_c_y,           &
                                ncell_fine,                &
                                coarse_proxy%data,         &
    ...
    !
    !$omp master
    call coarse_proxy%set_dirty()
    !$omp end master
    !
    !$omp end parallel
    !
  end subroutine invoke_restrict_kernel

  !----------------------------------------------------------------------------
  !> Handles the passing of coarse and fine fields to the prolong kernel.
  !> The mesh map shape arguments are not yet supported by PSyClone,
  !> where it is described by Issue #1173.
  !> https://github.com/stfc/PSyclone/issues/1173
  subroutine invoke_prolong_kernel(fine, coarse)
    ...
    ! Arguments
    type(field_type), intent(in) :: fine, coarse
    ! Internal variables
    integer(kind=i_def) :: cell
    integer(kind=i_def) :: nlayers
    integer(kind=i_def) :: ndf_fine, undf_fine, ndf_coarse, undf_coarse
    integer(kind=i_def) :: ncell_fine, ncell_f_per_c_x, ncell_f_per_c_y
    integer(kind=i_def), pointer :: map_fine(:,:) => null()
    integer(kind=i_def), pointer :: map_coarse(:,:) => null()
    integer(kind=i_def), pointer :: cell_map_coarse(:,:,:) => null()
    ...
    type(mesh_type),     pointer :: mesh_coarse => null()
    type(mesh_type),     pointer :: mesh_fine => null()
    type(field_proxy_type)       :: fine_proxy, coarse_proxy
    ...
    ! Initialise number of layers
    !
    nlayers = fine_proxy%vspace%get_nlayers()
    !
    ! Look-up mesh objects and loop limits for inter-grid kernels
    !
    mesh_fine => fine_proxy%vspace%get_mesh()
    ...
    !
    !$omp master
    call fine_proxy%set_dirty()
    !$omp end master
    !
    !$omp end parallel
    !
  end subroutine invoke_prolong_kernel
