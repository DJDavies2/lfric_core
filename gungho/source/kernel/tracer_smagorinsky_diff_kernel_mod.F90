...
subroutine tracer_smagorinsky_diff_code( nlayers,                               &
  ...
  real(kind=r_def), dimension(undf_wt),  intent(in)    :: shear

  ! Internal variables
  integer(kind=i_def) :: k
  real(kind=r_def)    :: d2dx, d2dy
  real(kind=r_def)    :: idx2
  real(kind=r_def)    :: visc_h
  ...
  do k = 1, nlayers - 1
    visc_h = ( (mix_factor * delta(map_wt(1) + k))**2) * shear(map_wt(1) + k)
    idx2 = 1.0_r_def/(delta(map_wt(1) + k))**2
    d2dx = (theta_n(map_wt_stencil(1,2) + k)  - 2.0_r_def*theta_n(map_wt_stencil(1,1) + k) +      &
            theta_n(map_wt_stencil(1,4) + k) )*idx2
    d2dy = (theta_n(map_wt_stencil(1,3) + k)  - 2.0_r_def*theta_n(map_wt_stencil(1,1) + k) +      &
            theta_n(map_wt_stencil(1,5) + k) )*idx2
    theta_inc(map_wt(1) + k) = visc_h*(d2dx + d2dy)
  end do
